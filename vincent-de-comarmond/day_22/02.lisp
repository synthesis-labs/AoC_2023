...
(defun chain-reaction (fallen-blocks supporters)
  (let ((block-ids (loop for block-id being the hash-keys of fallen-blocks
			 collect block-id))
	(total 0))

    (loop for id in block-ids do
      (let ((del-me (list id))
	    (grew 'T))
	...
	(loop while grew do
	  ...
	  (maphash (lambda (k v)
		     (if (null (set-difference v del-me))
			 (when (not (member k del-me))
			   (push k del-me)
			   (setf grew 'T))))
		   supporters))
	;; (format t "ID: ~s~%" id)
	;; (format t "Cascade size: ~s~%" (1- (length del-me)))
	(setf total (+ total (1- (length del-me))))))
    total))
...
(time (let* ((input (read-input "./input.txt"))
	...
	(format t "Chain reaction score: ~s~%" (chain-reaction fallen supporters))))

;; 488 is the right answer for part 1
;; run-time is like 0.05 s

;; 79465 is the correct answer for part 2
;; run-time (incl part 1) is like 0.3 s
