...
(defun read-input (input-fp)
  (let ((instructions '()))
	...
	(destructuring-bind (dir num colour) (split line #\space)
	  (declare (ignore dir num))
	  (push (list (case (char colour 7)
			(#\0 #\R) (#\1 #\D) (#\2 #\L) (#\3 #\U))
		      (parse-integer (subseq colour 2 7) :radix 16))
		instructions))))
...
(defun yolo (segments)
  (let* ((total 0)
    ...
    (loop for j from min-j to max-j do
      (if (= 0 (mod (- j min-j) 1000000))
	  (format t "% Complete: ~,2f~%" (* 100 (/ j (- max-j min-j)))))
      (let* ((vert-lines '())
...
;; Target for sample is:952408144115 
;; Sample iteration runs in 0.249 seconds

;; 60612092439765 is the right answer for part 2 (roughly 1 minute run time)
