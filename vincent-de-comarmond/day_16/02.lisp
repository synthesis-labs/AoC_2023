...
(defun all-energies (grid rows cols)
  (let ((starting-points '())
	(starting-directions '())
	(energies '()))

    (loop for i from 0 to (1- rows) do
      (push (cons i -1) starting-points)
      (push '(0 . 1) starting-directions)
      (push (cons i cols) starting-points)
      (push '(0 . -1) starting-directions))

    (loop for j from 0 to (1- cols) do
      (push (cons -1 j) starting-points)
      (push '(1 . 0) starting-directions)
      (push (cons rows j) starting-points)
      (push '(-1 . 0) starting-directions))

    (loop for start in starting-points for dir in starting-directions do
      (let* ((solution (solve-line-segments grid rows cols start dir))
	     (energized (determine-energized solution rows cols)))
	(push (length energized) energies)))
    energies))



(destructuring-bind (grid rows cols) (read-grid "./input.txt")
  (let ((energies (all-energies grid rows cols)))
    (format t "Max energy: ~s~%" (apply 'max energies))))

;; 7185 is the right answer for part 1
;; 7616 is teh right answer for part 2
